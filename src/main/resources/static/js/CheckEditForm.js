...
function checkAllEditForm()
{
    var pass1=$("#password").val();
    var pass2=$("#repeatedPassword").val();
    var email=$("#email").val();

    if(pass1.trim().length==0||pass2.trim().length==0||email.trim().length==0)
    {
        alert("Please fill in the information.");
        return false;
    }
    else if(!passwordIsValid()||!passwordIsSame()||!emailIsValid())
    {
        alert("Please fill in correct the information.");
        return false;
    }
    else
        return true;
